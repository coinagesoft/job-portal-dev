...
function CandidateForm() {
  ...
  const handlePay = () => {
    ...
    const completeAsDemo = () => {
      ...
      setPaymentMessage("Payment captured in demo mode. Add NEXT_PUBLIC_RAZORPAY_KEY_ID in .env.local for live checkout.");
      setLoading(false);
    };
    ...
    const options = {
      ...
      handler: function(response) {
        setPayStatus("success");
        setPaymentMessage(`Payment successful. Reference: ${response?.razorpay_payment_id || "N/A"}`);
        setLoading(false);
      },
    ...
    <div>
      ...
      {mobileOtp.verified && (
        <Field label="Registration Fee">
          ...
          {paymentMessage && (
            <p style={{ fontSize: 11, marginTop: 8, color: "var(--color-text-tertiary)" }}>{paymentMessage}</p>
          )}
        </Field>
      )}
...
function EmployerForm() {
  ...
  const sendCorpEmailOtp = () => {
    if (!data.corpEmail.includes("@") || data.corpEmail.includes("gmail") || data.corpEmail.includes("yahoo")) {
      showToast("Please use a corporate email — free domains like Gmail/Yahoo are not allowed", "warning");
      return;
    }
    const otp = genOtp();
